...
expect('index carrega o módulo competitivo v28', index.includes('assets/post-exam-competition-v28.js?v=33'));
...
expect('v28 liga o painel pós-prova', index.includes('assets/post-exam-follow-up-v28.js?v=33') && followUp.includes('ACOMPANHAMENTO PÓS-PROVA'));
...
expect('ordenação objetiva não usa desempate final', snapshot.postExam?.competitionReading?.rules?.objectiveOrdering?.includes('o item 16.7 não é usado como regra de ordenação desta lista objetiva') && competition.includes('O item 16.7 não é a regra de ordenação dessa lista.'));
expect('critérios do item 16.7 ficam restritos à nota final', snapshot.postExam?.competitionReading?.rules?.tieBreakScope?.includes('nota final do concurso público'));
expect('edital consolidado inclui a retificação 6', snapshot.postExam?.competitionReading?.sources?.updatedNotice?.includes('ae27ef17-6914-4b96-9f9d-c5c0c151d95a.pdf'));
...
expect('cronograma inclui resultado e recursos das fases complementares', snapshot.postExam?.followUp?.milestones?.some(item => item.id === 'biopsychosocial-hetero-preliminary' && item.date === '2027-02-12') && snapshot.postExam?.followUp?.milestones?.some(item => item.id === 'biopsychosocial-hetero-appeals' && item.start === '2027-02-15' && item.end === '2027-02-19') && snapshot.postExam?.followUp?.milestones?.some(item => item.id === 'biopsychosocial-hetero-definitive' && item.date === '2027-02-26'));
expect('cronograma inclui desempate provisório e recurso', snapshot.postExam?.followUp?.milestones?.some(item => item.id === 'tie-break-preliminary' && item.date === '2027-03-22') && snapshot.postExam?.followUp?.milestones?.some(item => item.id === 'tie-break-appeals' && item.start === '2027-03-23' && item.end === '2027-03-30'));
...
expect('cache v52 inclui estilos das fases, Radar Oficial e tratamento móvel', serviceWorker.includes("plano-transicao-v52-history-reconciliation") && serviceWorker.includes("'./assets/transition-pages-v29.css'") && serviceWorker.includes("'./assets/pre-post-v33.css'") && serviceWorker.includes("'./assets/navigation-mobile-v38.css'"));
