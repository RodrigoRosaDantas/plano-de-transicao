...
function examsView() {
  ...
  return `<div class="view-stack exams-view">${viewHeading("Concursos", "Prova, nota, classificação e contexto no lugar certo.", "A prova real funciona como régua externa. O treino não é usado para maquiar resultado de concurso.", `<button class="secondary-button" type="button" data-copy-exams>${svgIcon("copy")} Copiar histórico</button>`)}
    ...
    <section class="panel exam-matrix-panel"><div class="panel-heading"><div><span class="eyebrow">MATRIZ AUDITADA</span><h2>Comparação sem apagar o estágio do concurso</h2></div>${statusChip(`${measured.length} resultados reais`, "aqua")}</div><div class="subject-table-wrap"><table class="data-table exam-matrix"><thead><tr><th>Concurso</th><th>Data</th><th>Resultado</th><th>Aproveitamento</th><th>Classificação</th><th>Etapa / universo</th></tr></thead><tbody>${exams.map((exam) => `<tr><td><strong>${esc(exam.name)}</strong><small>${esc(exam.role)}</small></td><td>${dateBR(exam.date)}</td><td>${esc(exam.score || "—")}</td><td>${exam.rawAccuracy == null ? "—" : pct(exam.rawAccuracy)}</td><td>${(exam.finalClassification ?? exam.finalClassificationAC) ? `${fmt(exam.finalClassification ?? exam.finalClassificationAC)}º final` : exam.classification ? `${fmt(exam.classification)}º` : "—"}</td><td>${esc(exam.classificationStage || exam.status || "—")}${exam.objectiveClassification && (exam.finalClassification ?? exam.finalClassificationAC) ? `<small>Objetiva: ${fmt(exam.objectiveClassification)}º</small>` : ""}</td></tr>`).join("")}</tbody></table></div></section>
  </div>`;
}
...
function preExamView() {
  ...
  return `<div class="view-stack pre-exam-view">
    ${viewHeading("Pré-edital", "Radar pré-edital: TJDFT e SEEDF.", "A Pré-prova pronta para o próximo concurso permanece ativa em modo de prontidão. A estrutura permanece preparada enquanto o radar acompanha cargos, notícias e gatilhos oficiais; quando houver um novo edital, cada trilha receberá dados próprios sem misturar a preparação com o pós-prova da SEDES/DF.", `<button class="primary-button" type="button" data-view="strategy">${svgIcon("compass")} Ver critérios</button><button class="secondary-button" type="button" data-view="post-exam">${svgIcon("flag")} Acompanhar SEDES/DF</button>`)}
...
function postExamView() {
  ...
  // Um marco "current" é a publicação que está em acompanhamento agora.
  // O cartão não pode pular diretamente para a janela recursal seguinte nem
  // perder a data só porque o marco atual foi classificado como current.
  const nextMilestone = milestones.find((item) => item.status === "current" && item.date)
    || milestones.find((item) => item.status === "upcoming" && (item.date || item.start || item.end))
    || milestones.find((item) => item.status === "current" && (item.start || item.end))
    || {};
  const nextMilestoneDate = nextMilestone.date || nextMilestone.start || nextMilestone.end || "";
...
function financeView() {
  ...
  return `<div class="view-stack finance-view">${viewHeading("Investimentos", "Quanto a transição custa — e o que esse número pode dizer.", "Confirmado, estimado, pago, não confirmado e anulado continuam separados. Ausência de evidência não vira R$ 0,00.", `<button class="secondary-button" type="button" data-export-finance>${svgIcon("download")} Exportar filtrado</button>`)}
    ...
    <section class="metric-grid">${metricCard("Confirmado geral", money(state.data.financeSummary.totals.confirmed), `${fmt(state.data.financeSummary.totals.transactions)} lançamentos computáveis`, "lime")}${metricCard("Estimado", money(state.data.financeSummary.totals.estimated), "Fora do confirmado", "amber")}${metricCard("SEDES/DF", money(state.data.financeSummary.sedes.confirmed), `${fmt(state.data.financeSummary.sedes.transactions)} lançamentos computáveis`, "aqua")}${metricCard("Anuladas", fmt(annulled), "Preservadas só para auditoria", "coral")}</section>
...
function auditChecks() {
  ...
  return [
    ...
    ["Cobranças anuladas ficam fora dos totais", data.financeEntries.filter((entry) => entry.situation === "Anulada").every((entry) => !entry.countsInCycle && !Number(entry.confirmed || 0) && entry.paid == null)],
